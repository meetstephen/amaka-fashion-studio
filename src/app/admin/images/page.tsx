...
export default function AdminImagesPage() {
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newUrl = typeof data.url === "string" ? data.url : undefined;

      if (targetId !== null) {
        setImages((prev) =>
          prev.map((img) => (img.id === targetId ? { ...img, url: newUrl } : img))
        );
      } else {
        setImages((prev) => [
          {
  ...
  const saveEdit = () => {
    setImages((prev) =>
      prev.map((img) =>
        img.id === editingId ? { ...img, name: editName, category: editCategory } : img
      )
    );
    setEditingId(null);
  };
